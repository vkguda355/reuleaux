...
int main(int argc, char **argv)
{
    ...
    //std::string file = str(boost::format("ur5_r%d_reachability.h5") % resolution);
    ...
    if (argc == 2)
    {
        ...
        ROS_INFO("building the workspqce for all the configurations");
        //file = str(boost::format("ur5_r%d_reachability.h5") % resolution);
        filename = path + file;
    }
    ...
    else if (argc == 4)
    {
        ...
        else
        {
            resolution = atof(argv[1]);
            std::string str(argv[2]);
            desiredConfig = atoi (argv[3]);

            ROS_INFO("building the workspqce for specified configurations %d", desiredConfig);
            if(std::strchr(str.c_str(), '/'))
            {
                filename = argv[2];
            }
    ...
    while (ros::ok())
    {
        ...
        // The center of every voxels are stored in a vector ROS_INFO("Total no of spheres now: %lu", new_data.size());
        ...
        // stored.
        // vamsi :: Only the First joint solution is stored. We may need this solutions in the future. Otherwise we can show
        ...
        std::vector<int>configCount(8,0); //create a vector of counters for each configuration starting at 0 each
        ...
        for (MultiVector::iterator it = pose_col.begin(); it != pose_col.end(); ++it)
        {
            ...
            if (k.isIKSuccess(it->first, joints, solns, configsFound))
                ...
                //          ur_kinematics::convertPoseToTrans(it->first,Tee);
                //          ur_kinematics::inverse(Tee, q_sols,vfree[0]);
                //

                //add a loop to add the found configuration to the counter
//                for (int con = 0; con<8; con++)
//                {
//                    if(configsFound[con])configCount[con]++;
//                }

                if(desiredConfig >= 0 && desiredConfig < 8)
                {
                    if(configsFound[desiredConfig])
                    {
                ...
                else if (desiredConfig == -1)
                {

                    pose_col_filter.insert( std::make_pair( it->second, &(it->first)));
                    ik_solutions.push_back(joints);
                    //cout<<"Found ik solutions: "<< solns<<endl;
                    //cout<<it->first[0]<<" "<<it->first[1]<<" "<<it->first[2]<<" "<<it->first[3]<<" "<<it->first[4]<<" "<<it->first[5]<<" "<<it->first[6]<<endl;
                }
                else {
                    ROS_ERROR_STREAM("Hey!! You have specified a configuration that does not exist.");
                }
            }
        }
        ...
        //        for (int con =0; con< 8 ; con++)
        //        {
        //            ROS_INFO("Number of Poses Found for Configuration %d: %d", con ,configCount[con]);
        //        }
